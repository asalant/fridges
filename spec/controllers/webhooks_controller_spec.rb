...
describe WebhooksController do
  ...
  describe "POST sendgrid" do
    ...
    context "from unknown email address" do
      before do
        @params['from'] = 'unknown@example.com'
        Fridge.expects(:create!).with do |hash|
          hash[:email_from].should == @params['from']
    ...
    context "from user email address" do
      before do
        @params['from'] = users(:alon).email
        Fridge.expects(:create!).with do |hash|
